 ...
 * Parses experiment timeline data into table format for PPTX native table output
 */
...
function parseJavelinBoard(lines) {
  ...
  for (const line of lines) {
    ...
    // Check for date key header (e.g., "2024-01:")
    ...
    // Parse field values within experiment (indent >= 4)
    if (currentExperiment && indent >= INDENT.FIRST_LEVEL) {
  ...
  // Convert experiments to table format
  // Headers: ["項目", "2024-01", "2024-02", ...]
  // Rows: [["顧客の仕事", "...", "..."], ...]
  const headers = ['項目', ...experiments.map((exp) => exp.label)];
  const rows = FIELD_PATTERNS.map(({ key, label }) => [label, ...experiments.map((exp) => exp[key] || '')]);
