 ...
 * 3. node create-presentation.js part1 (フィルター指定)
 *
 * 環境変数:
...
async function buildPptx(filter, allSlideFiles, slidesDir) {
  ...
  console.log(`\n📦 ${filter || "全スライド"} (${slideFiles.length}枚) → ${outputFile}`);
...
async function createPresentation() {
  ...
  // コマンドライン引数からフィルター条件を取得（複数指定可能）
  const filterArgs = process.argv.slice(2);

  // 2_htmls/ 以下の HTML ファイルを取得（特殊ファイルを除外）
  // 除外: index.html, _preview.html, _で始まるファイル
